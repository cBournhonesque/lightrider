...
- `Dockerfile.matchmaker` defaults to low-memory release builds for the control image: one Cargo job, release LTO disabled, many codegen units, and `lld` for the native Bevygap binaries. This avoids local `rustc` SIGKILL/OOM failures when compiling `bevygap_matchmaker`; larger machines can override the build args through `MATCHMAKER_CARGO_JOBS`, `WEB_CARGO_JOBS`, and related env vars in the `just matchmaker-build` recipe.
...
- Reduced matchmaker/control image build pressure after local `bevygap_matchmaker` release builds were killed by SIGKILL. `Dockerfile.matchmaker` now defaults to `-j 1`, disables release LTO, raises release codegen units, uses low optimization for native control binaries, optimizes the WASM build for size, and installs `lld`; `just matchmaker-build` forwards env-overridable build args, and `SKIP_IMAGE_BUILD=1 just deploy-web-server host=<vps-ip> tag=<tag>` can reinstall an already-pushed image without rebuilding locally.
