...
- Fixed the rendered-client panic reported after magnetic food was added. The bad path was Lightyear interpolation on short-lived food `Position` components; food can despawn while interpolation history is being updated. Removed `Position.add_interpolation_with(...)` and `InterpolationTarget` from food, and replaced it with client-side renderer smoothing keyed by entity using `render.food_visual_lerp`.
- Made food smaller and added server-authoritative magnetic attraction. Food now has separate `visual_radius`, absorption `radius`, `magnet_radius`, and `magnet_speed` config fields; the server moves nearby food toward the closest same-room snake head before pickup checks, and the client renderer smooths food dots toward replicated positions.
...
- Verification for the food-interpolation panic fix: `cargo fmt --all`, `CARGO_INCREMENTAL=0 cargo check --workspace -j 4`, `CARGO_INCREMENTAL=0 cargo test --workspace --lib -j 4`, and `CARGO_INCREMENTAL=0 just trace-local 1 4 config/test.ron 5065` pass/complete. The trace at `logs/debug/20260526-205031` showed zero `snake_invariant_violation` and zero `server_late_input_mismatch` rows and no panic/error/despawn/update-confirmed-history markers. A rendered-client smoke was attempted, but this container/session cannot present a Bevy window reliably: Xvfb fails Vulkan presentation due missing DRI3, the existing X display segfaults before connection, and `WGPU_BACKEND=gl` reports no GPU.
