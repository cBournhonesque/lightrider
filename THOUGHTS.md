...
- Current asset-backed rendering covers the original grid background, wall-stretch arena outline, thin uniform snake tails, elongated directional snake heads, and tinted food sprites. It intentionally keeps behavior and interpolation unchanged.
- The background grid is rendered as one tiled Bevy sprite rather than many individual square sprites, so adjacent cells should merge without visible gaps. If stronger visual polish is needed later, the snake glow can move from layered sprites to a dedicated line/glow shader.
- Proximity boost has local visual feedback: the client uses the same ray-vs-tail geometry as movement boost detection and renders original Powerline spark/lightning frames at the tail being boosted against. High-speed snakes also emit original `particleDot` sprites near the head.
