...
- Production Dockerfiles use `cargo-chef` to split dependency and app build layers. The context includes `lightrider`, `bevygap`, and `lightyear`; manifest changes in any of them invalidate dependency recipes, while source-only changes should mostly hit final app build layers. Local image recipes use Podman `--layers` by default and accept optional `PODMAN_CACHE_FROM`/`PODMAN_CACHE_TO` for explicit remote cache experiments.
