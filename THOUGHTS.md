...
- Browser startup/status messages are split between static HTML and Bevy state. `web/index.html` shows `Loading assets...` before WASM initializes. After Bevy starts, `client::web_status` updates the same DOM element from `BevygapClientState`: `Waiting for new server to start` for fresh Edgegap deployments, `Waiting to connect to server` for warm deployments/connection, and hidden once Lightyear connects.
