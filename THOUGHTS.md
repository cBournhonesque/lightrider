...
- Current asset-backed rendering covers the original grid background, wall-stretch arena outline, glowing layered snake tails/heads, and tinted food sprites. It intentionally keeps behavior and interpolation unchanged.
...
- WASM browser clients install `bevy_web_keepalive::WebKeepalivePlugin` so the Bevy app can continue ticking when the browser tab is hidden. This is pinned to `Nul-led/bevy_web_keepalive` commit `ac1e853d378c1b6f4df22fdbbf29bb954146f4de` because the released crates.io `0.4.0` line targets Bevy 0.17, while the pinned git revision targets Bevy 0.18.
...
- Fixed first browser smoke-test polish issues after a successful real Edgegap join through local matchmaker/control: the web menu now has a close button, private/specific/new room selections show a persistent room badge, and the Bevy client now adds `bevy_web_keepalive` on wasm builds to reduce hidden-tab freezing. Also documented that Edgegap CPU metrics are not automatically fed into NATS. Verification: `cargo fmt --all`, `rustup run nightly cargo check -j 2 -p web_client --target wasm32-unknown-unknown --features bevygap`, `cargo check -j 2 -p client --features bevygap`, and `cargo check -j 2 -p server --features bevygap`.
