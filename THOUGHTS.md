...
- Sound uses sliced clips from the original `sounds/out.ogg` sprite: confirmed deaths play the crash sound, confirmed local food pickups play the grab sound, and speed loops use the original line-loop clips with volume driven by local speed.
...
- Config should cover arena dimensions, tick rate, spawn rules, respawn cooldowns, starting length, speed/boost tuning, food count, room capacity, bot count, fake-client defaults, render/debug camera knobs, tail width, map outline width, sound volumes/speed thresholds, network input delay, and debug tracing.
