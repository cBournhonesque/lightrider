...
use shared::network::protocol::{GameAction, GameProtocol, PlayerMovement};
...
impl Plugin for NetworkInputsPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_plugins(LeafwingInputPlugin::<GameProtocol, GameAction>::default());
...
fn handle_game_action(
    ...
    mut players: Query<(Entity, &mut Player, &ActionState<GameAction>)>
) {
    for (player_entity, mut player, action_state) in players.iter_mut() {
        if action_state.just_pressed(GameAction::Spawn) {
